...
namespace Mobile.Code.Services
{
    ...
    public class InvasiveVisualProjectLocationPhotoDataStore : IInvasiveVisualProjectLocationPhotoDataStore
    {
        ...
        public async Task<bool> AddItemAsync(VisualProjectLocationPhoto item, bool isOffline = false)
        {
            ...
            var multiImage = new MultiImage() { Id = item.Id, Image = item.ImageUrl, ParentId = item.VisualLocationId, Status = "New", IsServerData = false, ImageType = item.ImageDescription };
            items.Add(item);
        ...
        public async Task<bool> UpdateItemAsync(VisualProjectLocationPhoto item, bool IsEditVisual)
        {
            ...
            if (oldITRaktem != null)
            {
                ...
                var updateMulti = new MultiImage() { ImageType = item.ImageDescription, Id = item.Id, Image = item.ImageUrl, ParentId = item.VisualLocationId, Status = "Update", IsServerData = true };
        ...
        public async Task<bool> DeleteItemAsync(VisualProjectLocationPhoto item, bool IsEditVisual)
        {
            ...
            {
                App.VisualEditTrackingForInvasive.Remove(oldITRaktem);
                var delMulti = new MultiImage() { ImageType = item.ImageDescription, Id = item.Id, Image = item.ImageUrl, ParentId = item.VisualLocationId, Status = "Delete", IsDelete = true, IsServerData = true };
