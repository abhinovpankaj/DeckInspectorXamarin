...
namespace Mobile.Code.Services.SQLiteLocal
{
    public class BuildingApartmentImagesSqLiteDataStore : IBuildingApartmentImages
    {
        ...
        public BuildingApartmentImagesSqLiteDataStore()
        {
            _connection = DependencyService.Get<ISQLite>().GetConnection();
            _connection.CreateTable<BuildingApartmentImages>();
        ...
        public Task<bool> AddItemAsync(BuildingApartmentImages item)
        {
            ...
            try
            {
                var buildingApartment = new BuildingApartmentImages
                {
        ...
        public Task<Response> DeleteItemAsync(BuildingApartmentImages item)
        {
            ...
            try
            {
                _connection.Delete<BuildingApartmentImages>(item.Id);
        ...
        {
            return Task.FromResult(_connection.Table<BuildingApartmentImages>().FirstOrDefault(t => t.Id == id));
        }

        public Task<IEnumerable<BuildingApartmentImages>> GetItemsAsync(bool forceRefresh = false)
        {
            return Task.FromResult(
                from t in _connection.Table<BuildingApartmentImages>()
                select t
                    );
        }
        ...
        {
            items = (from t in _connection.Table<BuildingApartmentImages>().Where(x => x.BuildingApartmentId == ApartmentID)
                     select t).ToList();
