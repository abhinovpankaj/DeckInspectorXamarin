...
{
    public class EditBuildingApartmentImageViewModel : BaseViewModel
    {
        ...
        private BuildingApartmentImages _buildinAprtImage;

        public BuildingApartmentImages BuildingApartmentImages
        {
            get { return _buildinAprtImage; }
            set { _buildinAprtImage = value; OnPropertyChanged("BuildingApartmentImages"); }
        }

        private async Task Save()
        {
            if (string.IsNullOrEmpty(BuildingApartmentImages.Name))
            {
                await Shell.Current.DisplayAlert("Alert", "Project name is required", "OK");
                return;
            }
            if (string.IsNullOrEmpty(BuildingApartmentImages.Id))
            {
                BuildingApartmentImages.Id = Guid.NewGuid().ToString();

                BuildingApartmentImages.ApartmentID = BuildingApartment.Id;
                ////Project.
                ////   Project.ProjectImage = ImgPata;

                await BuildingApartmentImagesDataStore.AddItemAsync(BuildingApartmentImages);
                await Shell.Current.Navigation.PopAsync();
            ...
            {
                await BuildingApartmentImagesDataStore.UpdateItemAsync(BuildingApartmentImages);
                await Shell.Current.Navigation.PopAsync();
            }
        ...
        private BuildingApartment _bapartment;

        public BuildingApartment BuildingApartment
        {
            get { return _bapartment; }
            set { _bapartment = value;OnPropertyChanged("BuildingApartment"); }
        }

        public EditBuildingApartmentImageViewModel()
        {
        ...
        {
            BuildingApartmentImages.ImageUrl = ImgData.Path;
